...
class WP_Post_Comment_Rating_Common {
	...
	public static function wpcr_save_comment_meta_data( $comment_id ) {
		$rating =  (empty($_POST['rating'])) ? FALSE : $_POST['rating'];
		add_comment_meta( $comment_id, 'rating', $rating );
	}
	
	/* VALIDATION */
	public static function wpcr_verify_comment_meta_data( $commentdata ) {
	...
	**/
	...
	public static function wpcr_show_nav_links($html){
		...
		if(is_single()){
			...
			$wpcr_socialsh = isset($wpcr_options['wpcr_social']) ? $wpcr_options['wpcr_social'] : false;
			...
			$facebookURL = 'https://www.facebook.com/sharer.php?s=100&amp;p[url]='.$wpcr_URL.'&p[title]='.$wpcr_Title.'&amp;p[images][0]='.$wpcr_Thumb.'';
			...
			if($navval == 1){
				...
				if($wpcr_socialsh == 1){
					...
					$html .= '<div class="wpcr_ss_links"><ul>';				
	...
	public static function wpcr_meta_tags_social(){
			...
			$results = get_option('wpcr_settings');
			$wpcr_social = isset($results['wpcr_social']) ? $results['wpcr_social'] : false;		
			...
			$wpcr_Thumbnail = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
			if($wpcr_Thumbnail){
				$wpcr_sl_Thumb = $wpcr_Thumbnail[0];
			}else{
				$wpcr_sl_Thumb = '';
			}
			...
			$post_content = wp_strip_all_tags($post_obj->post_content);
				...
				if($wpcr_social == 1){		
					...
					$html .= '<meta itemprop="description" content="'.$post_content.'">';
					$html .= '<meta itemprop="image" content="'.$wpcr_sl_Thumb.'">';
					
					$html .= '<meta name="twitter:card" content="summary">';
					$html .= '<meta name="twitter:title" content="'.$wpcr_Title.'">';
					$html .= '<meta name="twitter:description" content="'.$post_content.'">';
					$html .= '<meta name="twitter:image" content="'.$wpcr_sl_Thumb.'">';
					
					$html .= '<meta property="og:title" content="'.$wpcr_Title.'" />';
					$html .= '<meta property="og:type" content="article" />';
					$html .= '<meta property="og:url" content="'.$wpcr_URL.'" />';
					$html .= '<meta property="og:image" content="'.$wpcr_sl_Thumb.'" />';
					$html .= '<meta property="og:description" content="'.$post_content.'" />';
					echo $html;
				}
				endif;
		}
